...
def get_norm_layer(norm_type='instance', dimension=2):
    """Return a normalization layer

    Parameters:
        norm_type (str) -- the name of the normalization layer: batch |
        instance | none

    For BatchNorm, we use learnable affine parameters and track running
    statistics (mean/stddev).
    For InstanceNorm, we do not use learnable affine parameters. We do not
...
def get_scheduler(optimizer, opt):
    ...
    Parameters:
        ...
        opt.lr_policy is the name of learning rate
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    support); 2. initialize the network weights
    Parameters:
...
def add_sn(m):
    add_list = (nn.Conv2d, nn.ConvTranspose2d, nn.Conv3d, nn.ConvTranspose3d)
    if isinstance(m, add_list):

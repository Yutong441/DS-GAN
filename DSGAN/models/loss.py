...
def nmse(x1, x2):
    numer = torch.sum((x1 - x2)**2) + 1e-10
    denom = torch.sum(x1**2) + 1e-10
    return torch.mean(torch.sqrt(numer/denom))
...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real):
        ...
        elif self.gan_mode in ['wgan', 'wgangp']:
            if target_is_real:
...
def cal_gradient_penalty(netD, real_data, fake_data, type='mixed',
    ...
    if lambda_gp > 0.0:
        if type == 'real':
            # either use real images, fake images, or a linear interpolation of
            # two.

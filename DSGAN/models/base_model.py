...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        Then, you need to define four lists:
            ...
            optimizers. You can define one optimizer for each network. If two
            networks are updated at the same time, you can use itertools.chain
            to group them. See cycle_gan_model.py for an example.
        """
        self.opt = opt
        ...
        self.device = torch.device(
            ...
            ) if self.gpu_ids[0] != "cpu" else torch.device("cpu")
    ...
    def setup(self, opt):
        ...
        if opt.isTrain:
            ...
            self.starting_epoch = self.schedulers[1].state_dict()["last_epoch"]
            self.starting_epoch = int(self.starting_epoch//opt.n_samples)
    ...
    def compute_visuals(self):
        """Calculate additional output images for visdom and HTML
        visualization"""
        pass
    ...
    def get_current_losses(self):
        ...
        write_mode, header = logger.write_csv_mode(self.opt.save_dir +
                                                   "/loss.csv")
        errors_ret = pd.DataFrame(errors_ret, index=[0])
        errors_ret.round(4).to_csv(self.opt.save_dir+"/loss.csv",
                                   mode=write_mode, header=header)

    def save_metrics(self, savepath, save_full=False):
        all_mets = pd.DataFrame.from_records(self.all_met)
        if not save_full:
            all_mets = all_mets.drop(columns=["ID", "dataset"])
            mets_mean = np.mean(all_mets.abs().values, axis=0)
            all_mets = pd.DataFrame(mets_mean, columns=[0],
                                    index=all_mets.columns).T
        # empty the cache
        self.all_met = []

        write_mode, header = logger.write_csv_mode(self.opt.save_dir+"/" +
                                                   savepath)
        all_mets.round(5).to_csv(self.opt.save_dir+"/"+savepath,
                                 mode=write_mode, header=header)
    ...
    def save_networks(self, epoch):
        ...
        Parameters:
            ...
            % (epoch, name)
        """
        proceed = logger.better_epoch(self.opt)
        if proceed:
            logger.write_log(self.opt, "saving parameters")
            for name in self.model_names:
                if isinstance(name, str):
            ...
            for index, i in enumerate(self.schedulers):
                spath = self.save_dir+"/schedulers/S{}.pth".format(index)
                torch.save(i.state_dict(), spath)
    ...
    def add_noise(self, x):
        if self.opt.disc_noise > 0:
            # noise_size = [x.shape[0], self.opt.input_nc, *x.shape[3:]]
            # nnoise_size = [x.shape[0], self.opt.output_nc, *x.shape[3:]]
            # args = {"dtype": x.dtype, "device": x.device}
            # const = torch.zeros(*nnoise_size, **args)
            # noise = torch.randn(*noise_size, **args
            #                     )*x.max()*self.opt.disc_noise
            # mult = torch.cat(const, noise)
            # return mult + x
            noise = torch.randn(*x.shape, dtype=x.dtype, device=x.device)
            return x.max()*self.opt.disc_noise*noise + x
        else:
            return x
    ...
    def load_networks(self, epoch):
        ...
        if self.opt.isTrain:
            for index, i in enumerate(self.schedulers):
                spath = self.save_dir+"/schedulers/S{}.pth".format(index)
                state_dict = torch.load(spath, map_location=str(self.device))
                i.load_state_dict(state_dict)
    ...
    def print_networks(self, verbose):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if verbose:
                    print(net)
                logger.write_log(self.opt,
                                 "Total number of parameters " + name +
                                 ": %.3f M" % (num_params / 1e6))

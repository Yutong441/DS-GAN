...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if opt.isTrain and self.opt.lambda_identity > 0.0:
            # if identity loss is used, we also visualize idt_B=G_A(B) ad
            # idt_A=G_A(B)
        ...
        if opt.isTrain:
            if opt.lambda_identity > 0.0:
                # only works when input and output images have the same number
                # of channels
    ...
    def backward_G(self):
        ...
        self.loss_L1_A = self.criterionCycle(self.real_B,
                                             self.fake_B) * lambda_L1
        self.loss_L1_B = self.criterionCycle(self.real_A,
                                             self.fake_A) * lambda_L1
        self.NMSE_A = loss.nmse(self.real_B,
                                self.fake_B) * self.opt.lambda_NMSE
        self.NMSE_B = loss.nmse(self.real_A,
                                self.fake_A) * self.opt.lambda_NMSE
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + \
            self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + \
            self.loss_L1_A + self.loss_L1_B + self.NMSE_A + self.NMSE_B
        self.loss_G.backward()

    def optimize_parameters(self, index):
